...
public class CustomPrintStream extends PrintStream {
	private final ThreadLocal<ByteArrayOutputStream> out; // Standard output stream for each thread
	private final ThreadLocal<Boolean> trouble; // Whether the standard output writing process of each thread throws IOException

	private final static int OUTPUT_LIMIT = 1024 * 10; // We are allowing 10 KB text
	...
	public String toString() {
		if (out.get() != null) {
			...
			return "Turns out you running code on a different thread, we don't have visibility over it.";
		}
	}
	...
	private void ensureLimit() throws IOException {
		var size = out.get().size();
		if (size >= OUTPUT_LIMIT) {
			//out.get().write("\nOutput Limit reached.".getBytes());
			throw new OutputLimitReachedException("Output Limit reached.");
		}
	}
